...
export default function ClaimForm() {
  ...
  // Re-sync if URL params change after mount
  useEffect(() => {
    if (prefillShop && !form.shop_name) {
      setForm(f => ({ ...f, shop_name: prefillShop }));
    }
  }, [prefillShop, form.shop_name]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input required className={INPUT} value={form.owner_name} onChange={(e) => set('owner_name', e.target.value)} placeholder="Jane Doe"/>
      </div>
      <div>
        <label className={LABEL}>Email Address *</label>
        <input required type="email" className={INPUT} value={form.email} onChange={(e) => set('email', e.target.value)} placeholder="jane@example.com"/>
      </div>
      <div>
